...
也可以复制 [.env.example](./.env.example) 为 `.env`。SiliconFlow 模型默认是 `BAAI/bge-reranker-v2-m3`，请求会把去标识化病例指纹以及候选题名/摘要发送到远程服务；使用前应确认机构的数据出境、隐私和服务条款要求。API key 只从环境变量或被 git 忽略的 `.env` 读取，不会写入结果 JSON、Markdown 或错误信息。
